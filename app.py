...
def should_chunk_stream(chunk, metadata) -> bool:
    '''
    THis prevents raw tool/serach/Rag json from appearing in the ui
    
    we wil only stream normal ai text chunks
    we do not stream 
    -Tool Message
    ...
    addtional_kwargs = getattr(chunk, "additional_kwargs", {}) or {}
    
    if addtional_kwargs.get("tool_calls"):
        return False
    ...
    def event_generator():
        ...
        try:
            ...
            for chunk, metadata in agent.stream(
                ...
                if not should_chunk_stream(chunk, metadata):
                    continue
